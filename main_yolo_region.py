...
def run(
    ...
    while videocapture.isOpened():
        ...
        region_tracks_this_frame = [set() for _ in counting_regions]
        ...
        if results[0].boxes.is_track:
            ...
            for box, track_id, cls in zip(boxes, track_ids, clss):
                ...
                for idx, region in enumerate(counting_regions):
                    if region["polygon"].contains(Point((bbox_center[0], bbox_center[1]))):
                        region["counts"] += 1
                        region_tracks_this_frame[idx].add(track_id)

        # For each region, update entry/exit times
        for idx, region in enumerate(counting_regions):
            prev_tracks = region["current_tracks"]
            curr_tracks = region_tracks_this_frame[idx]
            # Tracks that just entered
            entered = curr_tracks - prev_tracks
            for track_id in entered:
                region["track_entries"][track_id] = vid_frame_count
            # Tracks that just exited
            exited = prev_tracks - curr_tracks
            for track_id in exited:
                entry_frame = region["track_entries"].pop(track_id, None)
                if entry_frame is not None:
                    duration = vid_frame_count - entry_frame
                    region["track_durations"].append(duration)
    ...
    for region in counting_regions:
        ...
        if durations:
            ...
            print(f"Region '{region['name']}': Longest stay = {longest/fps:.2f}s, Average stay = {average/fps:.2f}s")
        else:
